...
BASE_CONFIG = SimpleNamespace(
    ...
    rnn_type="gru",           # set to best cell from previous results, change if needed
    num_layers=2,
    bidirectional=False,
    dropout=0.0,
    vocab_max_size=-1,
...
def grid_search(best_rnn_type):
    ...
    with open(RESULTS_FILE, "w") as f, open(CSV_FILE, "w", newline="") as csvfile:
        ...
        csv_writer.writerow(["model", "val_acc", "val_loss", "vocab_min_freq", "hidden_dim", "lr", "clip", "freeze"])
        ...
        for vocab_min_freq, hidden_dim, lr, clip, freeze in product(vocab_min_freqs, hidden_dims, lrs, clips, freezes):
            ...
            results['rnn'].append((val_acc, val_loss, vocab_min_freq, hidden_dim, lr, clip, freeze))
            f.write(f"RNN | vocab_min_freq={vocab_min_freq}, hidden_dim={hidden_dim}, lr={lr}, clip={clip}, freeze={freeze} -> val_acc={val_acc:.4f}, val_loss={val_loss:.4f}\n")
            csv_writer.writerow(["rnn", val_acc, val_loss, vocab_min_freq, hidden_dim, lr, clip, freeze])
        ...
        for vocab_min_freq, hidden_dim, lr, clip, freeze in product(vocab_min_freqs, hidden_dims, lrs, clips, freezes):
            ...
            val_loss, val_acc = run_single_config(config)
            results['baseline'].append((val_acc, val_loss, vocab_min_freq, hidden_dim, lr, clip, freeze))
            f.write(f"Baseline | vocab_min_freq={vocab_min_freq}, hidden_dim={hidden_dim}, lr={lr}, clip={clip}, freeze={freeze} -> val_acc={val_acc:.4f}, val_loss={val_loss:.4f}\n")
            csv_writer.writerow(["baseline", val_acc, val_loss, vocab_min_freq, hidden_dim, lr, clip, freeze])
        ...
        f.write("\n=== Best RNN Config ===\n")
        f.write(f"val_acc={best_rnn[0]:.4f}, val_loss={best_rnn[1]:.4f}, vocab_min_freq={best_rnn[2]}, hidden_dim={best_rnn[3]}, lr={best_rnn[4]}, clip={best_rnn[5]}, freeze={best_rnn[6]}\n")

        f.write("\n=== Best Baseline Config ===\n")
        f.write(f"val_acc={best_baseline[0]:.4f}, val_loss={best_baseline[1]:.4f}, vocab_min_freq={best_baseline[2]}, hidden_dim={best_baseline[3]}, lr={best_baseline[4]}, clip={best_baseline[5]}, freeze={best_baseline[6]}\n")
...
def evaluate_best_configs(best_rnn_config, best_baseline_config, best_rnn_type):
    ...
    with open(RESULTS_FILE, "a") as f:
        ...
        for seed in seeds:
            ...
            config.model_type = "rnn"
            config.rnn_type = best_rnn_type
            config.vocab_min_freq = best_rnn_config[2]
            config.hidden_dim = best_rnn_config[3]
            config.lr = best_rnn_config[4]
            config.clip = best_rnn_config[5]
            config.freeze_embeddings = best_rnn_config[6]
            config.seed = seed
        ...
        for seed in seeds:
            config = SimpleNamespace(**vars(BASE_CONFIG))
            config.model_type = "baseline"
            config.vocab_min_freq = best_baseline_config[2]
            config.hidden_dim = best_baseline_config[3]
            config.lr = best_baseline_config[4]
            config.clip = best_baseline_config[5]
            config.freeze_embeddings = best_baseline_config[6]
            config.seed = seed
