  ...
  for epoch in range(1, max_epochs+1):
    ...
    for i in range(num_batches):
      ...
      if i % 100 == 0:
        ...
        print(f"Conv1 weight L2 norm: {np.linalg.norm(net[0].weights):.4f}")
        print(f"Conv2 weight L2 norm: {np.linalg.norm(net[3].weights):.4f}")
        print(f"FC weight L2 norm: {np.linalg.norm(net[7].weights):.4f}")
      if i > 0 and i % 50 == 0:
