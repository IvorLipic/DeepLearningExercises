...
weight_decays = [1e-2]
config['weight_decay'] = weight_decays[0]
...
def init_model_arch(config):
    ...
    return net, regularizers, loss

for wd in weight_decays:
    config["weight_decay"] = wd
    net, regularizers, loss = init_model_arch(config)
    print(f"Testing model reg. factor: {wd}")
