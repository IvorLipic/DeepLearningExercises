...
class FC(Layer):
  def __init__(self, input_layer, num_outputs, name,
    ...
    self.bias = bias_initializer_fn([1, num_outputs])
    self.name = name
    self.has_params = True
  ...
  def forward(self, inputs):
    ...
    """
    self.x = inputs
    return np.dot(inputs, self.weights.T) + self.bias # y = x * W.T + b
  ...
  def backward_inputs(self, grads):
    ...
    """
    # grads == dL / dy
    return np.dot(grads, self.weights) # dL/dx = W * dL / dy
  ...
  def backward_params(self, grads):
    ...
    """
    # grads == dL / dy
    grad_weights = np.dot(grads.T, self.x) # dL / dW = (dL / dy) * x.T
    grad_bias = np.sum(grads, axis=0, keepdims=True) # dL / db = (dL / dy)
    return [[self.weights, grad_weights], [self.bias, grad_bias], self.name]
...
class SoftmaxCrossEntropyWithLogits():
  ...
  def softmax(self, x):
    ...
    probs = np.clip(exp_x / np.sum(exp_x, axis=1, keepdims=True), a_min=1e-12, a_max=1.)
    return probs
  ...
  def forward(self, x, y):
    ...
    """
    return -np.sum(y * np.log(self.softmax(x))) / x.shape[0]
  ...
  def backward_inputs(self, x, y):
    ...
    """
    return (self.softmax(x) - y) / x.shape[0]
